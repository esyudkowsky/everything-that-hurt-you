...
- 2026-07-06 (voiceover lifecycle REFACTOR + @fadeout, by Fable): author reported the bubble->black transition ("...one of them is this:" -> "Always two they are...") still dropped the caption before the dragon hoard faded, and asked for a refactor of the overcomplicated system such that whole-scene fades are easy. New model in engine.js:
  - **One lifecycle rule**: #volayer (backdrop + box + text) always fades AS ONE UNIT; content cleared only once fully hidden (hideVoLayer's timer). helpers: cancelVoHide/showVoLayer/hideVoLayer/setVoiceover — one path each.
  - **All active @voiceover modes are now deferred** (st.voDeferred + voDeferredPc; layer-show + prerender happen at the first line's reveal). For "on" the reveal runs in the same step-loop tick (backdrop fades in immediately); for "over"/"bubble" it runs on the next click. A preceding fade-out therefore always completes untouched. This REPLACED the special-cased eager "on" path.
  - **"on" now FADES the scene out under its incoming black** (clearCg("fade"); cut only for instant seek reconstruction) — previously cut.
  - **New script directive `@fadeout [ms]`** (default 800): fades the WHOLE scene to black together — volayer + bglayer imagery (transitionDuration override, unison) + sprites — then clears all once dark; scene-image cleanup on an independent timeout so a following @voiceover can't cancel it. Parser + exec + seek + prerenderVoGroup group-terminator + validator (fadeout sets vo=off in both tracking loops) all updated. Documented in script.txt header.
  - **script.txt**: the bubble->on beat is now `@fadeout` + `@pause 850` + `@voiceover on` (echoed as a note in script.md). Verified headless: voOp === bgOp at every sample (0.033/0.033 mid-fade) — caption and hoard sink in exact lockstep; black beat; "Always two" fades in centred.
  - **Regressions all green**: white-patch (off->dragon->bubble), ch1 over-wash, on->dragon mirrored crossfade (vo 1->0.55->0 vs bg 0->0.58->1), validator 0 errors, FULL e2e click-through (699 clicks, end card, resume, persistence, ch12 jump). e2e.js had a stale "The Cliff" chapter-jump selector from the old 14-chapter numbering — fixed to "Truth"; shots/ restored per convention.

- 2026-07-06 (link-preview / social metadata): index.html <head> had no Open Graph / Twitter Card tags, so pasting the URL (e.g. in Discord) showed only the bare title. Added og:type/site_name/title/description/url/image(+width 1376/height 768/alt), twitter:card=summary_large_image (+title/description/image), a meta description, canonical, favicon + apple-touch-icon (assets/ui/ui_title.png), and theme-color #100c08. Preview image = the title art (assets/ui/ui_title.png, 1376x768 ~1.79:1 → large card). URLs are ABSOLUTE to https://everything-that-hurt-you.vercel.app/ (scrapers fetch them directly; relative would break). The CSP does NOT need changing — it governs the page's own loads, not the scraper. GOTCHA: Discord/Slack/etc. cache previews hard; an already-shared link won't refresh for a while — test with a fresh link or a ?v=N cache-buster, and validate via a link-preview debugger.

- 2026-07-06 (link-preview image = the TITLE SCREEN, not bare art): author wanted the preview to show the whole title page as a player sees it ("this is a kinetic novel you can play"), not just the background art. New asset assets/ui/og_title.png = a fresh-first-visit screenshot of the title screen (title + "a kinetic novel · Eliezer Yudkowsky" + content warning + Begin/Settings/Credits over the art), captured headless at 1280x720 @2x then downscaled to 1920x1080. og:image + twitter:image now point to it (favicon stays ui_title.png). NOTE: kept it FULL 16:9 — a 1.91:1 crop clips content because the title screen spans the whole height (Ch-indicator HUD at the very top, Credits button at the very bottom); cropping from the top cut "Ch. 0/13", cropping from the bottom cut "Credits". Discord shows the full og:image uncropped so 16:9 is ideal there; X/FB will center-crop slightly (unavoidable for full-height content). To regenerate: tools/... or the ogcard.js pattern in scratch — fresh localStorage, screenshot #stage, resize to 1920x1080.

- 2026-07-06 (dedicated link-preview CARD, supersedes the raw screenshot): frontline perception, worth the effort — replaced the full-title-screen screenshot with a purpose-composed card in Discord's preferred 1.91:1 (1200x630). assets/ui/og_title.png is now a COMPOSITION (not a screenshot): the title art (ui_title.png) under a legibility gradient, "EVERYTHING THAT HURT YOU" in Cinzel (two lines), and a deliberately LARGER byline — "a kinetic novel" (italic Georgia ~37px) + "Eliezer Yudkowsky" (Cinzel ~44px) so they read at thumbnail size — plus a game-styled "Begin ▶" button held ~64px off the bottom edge. Reuses the site's real fonts (--vo Cinzel self-hosted, --serif Georgia) and button styling. Generator kept at **claude-notes/tools/ogcard.html** (root-absolute /assets/ paths). REGEN: serve project root, `playwright` screenshot http://localhost:8321/claude-notes/tools/ogcard.html at viewport 1200x630 @2x -> downscale to 1200x630 (see scratch rendercard.js). og:image + twitter:image + og:image:width/height(1200/630) updated; favicon still ui_title.png. Do NOT put the generator in the web root (it would be publicly served).

- 2026-07-08 (voice-acting direction experiment): author asked for (a) a comment-free copy of
  script.txt (many @note comments are bad/obsolete) — `claude-notes/script-nocomments.txt`;
  (b) `claude-notes/voice-script.md`: for each dialogue line, Fable's guess at what the character
  was *actually feeling* and, separately where applicable, *what they were trying to outwardly
...
- 2026-07-16 (player-feedback batch, relayed by author): (1) podium MICROPHONE anachronism in Ch.3 auction — confirmed present in BOTH bg_auction_plaza.png and bg_auction_plaza_forsale.png (gooseneck mic on the podium); auction CGs are clean. (2) THIRD HAND in Ch.6 campfire cuddle (cg_campfire_shoulder.png) — confirmed: her two hands stacked on her knee PLUS an extra clawed hand gripping Avram's blanket at his arm. (3) Title screen: REMOVE the "benefits from being read twice" claim (engine.js post-finish note), KEEP "Please do not spoil it for others." (4) Bug: dialogue text size sometimes GROWS mid-read and the last words overflow/hide below the textbox (player screenshot: phone, Ch.2 bartender scene). (5) Phone landscape hides bottom UI (browser chrome, no fullscreen button); player works around by rotation-lock portrait. (6) Nameplate ("namelabel div") spills downward at very wide/short viewports (~2000x400 screenshot). (7) Player wants quick mousewheel back-scroll.
- 2026-07-16 (author's design direction on quick-menu request): player asked for industry-standard buttons (history/chapter/etc.) beside the dialogue box + right-click context menu. Author's take, delegated to Claude's judgment: check industry standard; consider a Settings option to hide such buttons, or a SINGLE expanding button instead of a full row; do NOT implement right-click if it would cost other functionality users want. Claude's decision (this session): one unobtrusive expander at the right end of the textbox revealing Back / History / Fullscreen / Menu; wheel-up = step back one line, wheel-down = advance (Ren'Py standard); no right-click hijack. Nameplate spill + wheel back-scroll were explicitly greenlit as "fine to just do/fix".

- 2026-07-16 (bandwidth + analytics decisions): Vercel bandwidth running out (though traffic died down since early release). Assets measured at 254 MB of PNG (engine already only preloads a few scenes ahead). Author decisions: (1) recompress art to WebP (in progress, reversible, PNGs kept); (2) hosting = KEEP Vercel for the app, move MEDIA to Cloudflare R2 (ChatGPT-suggested hybrid; content-hashed filenames + immutable cache headers); (3) reader-progress analytics = Cloudflare Worker + Analytics Engine — anonymous localStorage reader id, beacon per new chapter reached + finish + exact max line (pc) on pagehide, goal: notice sudden drop-off points. New OpenRouter key provided this session ($20; author explicitly fine with low-security handling, kept out of tracked files).
- 2026-07-16 (R2 domain): author does NOT currently own a custom domain for this story ("probably I should, but at present I don't") → R2 media will be served from the free r2.dev public bucket URL for now (rate-limited, not officially production-recommended — acceptable at current traffic). Engine asset-base is a single constant, so upgrading to media.<domain> later = one-line change + DNS.
- 2026-07-17 (podium mic, round 2): player/author caught the SAME anachronistic gooseneck microphone in the two ownership-transfer CGs (cg_ownership_transfer.png, cg_ownership_dark.png) — on the lectern behind the clerk, upper-center. Fixed identically (NB Pro edit, 1 attempt each, clean); originals backed up to claude-notes/raw/pre-fix-2026-07-17/. LESSON: the mic was a recurring motif of the original art job wherever a podium/lectern appears — if any FUTURE podium/lectern art shows up, check for it. Known clean: auction CGs. v1.1.1.
